...
object Mysql2HiveOdsPayment extends SparkSessionWrapper {
  ...
  def writeToHive(df: DataFrame): Unit = {
    df.write
      ...
      .partitionBy("dt")
      .format("parquet")
      .saveAsTable("ods.mysql2hive_ods_payment_df")
  }

  def backupData(df: DataFrame): Unit = {
    val backupPath = s"${config.getString("etl.paths.archive")}/payments/${LocalDateTime.now.format(DateTimeFormatter.ofPattern("yyyyMMdd"))}"
