...
object Mysql2HiveOdsCategory extends SparkSessionWrapper {
  ...
  def writeToHive(df: DataFrame): Unit = {
    df.write
      ...
      .partitionBy("dt")
      .format("parquet")
      .saveAsTable("ods.mysql2hive_ods_category_df")
  }

  def backupData(df: DataFrame): Unit = {
    val backupPath = s"${config.getString("etl.paths.archive")}/product_categories/${LocalDateTime.now.format(DateTimeFormatter.ofPattern("yyyyMMdd"))}"
