...
object Mysql2HiveOdsUserCoupon extends SparkSessionWrapper {
  ...
  def writeToHive(df: DataFrame): Unit = {
    df.write
      ...
      .partitionBy("dt")
      .format("parquet")
      .saveAsTable("ods.mysql2hive_ods_user_coupon_df")
  }

  def backupData(df: DataFrame): Unit = {
    val backupPath = s"${config.getString("etl.paths.archive")}/user_coupons/${LocalDateTime.now.format(DateTimeFormatter.ofPattern("yyyyMMdd"))}"
